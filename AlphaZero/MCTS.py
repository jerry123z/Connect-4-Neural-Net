...
import AlphaZero.   board_encoder_decoder as ed
import copy
import torch
import torch.multiprocessing as mp
from AlphaZero.AlphaZeroNet import AlphaZero
import datetime
import logging
...
class UCTNode:
    ...
    def expand(self, child_priors):
        ...
        action_idxs = self.game.get_legal_actions()
        c_p = child_priors
        if not action_idxs:
            self.is_expanded = False
    ...
    def backprop(self, value_estimate: float):
        current = self
...
def UCT_search(game_state, num_reads, net, temp):
    ...
    for i in range(num_reads):
        ...
        if leaf.game.check_win() != 0 or not leaf.game.get_legal_actions():  # if somebody won or draw
...
def MCTS_self_play(alphaZero, num_games, start_idx, cpu, args, iteration):
    ...
    for idxx in tqdm(range(start_idx, num_games + start_idx)):
        ...
        current_board = Connect4GameBoard()
        checkmate = False
        dataset = []  # to get state, policy, value for neural network training
        states = []
        value = 0
        move_count = 0
        while not checkmate and current_board.get_legal_actions() != []:
            if move_count < 11:
            ...
            root = UCT_search(current_board, 1000, alphaZero, t)
            ...
            print(" ")
            if current_board.check_win() != 0:  # if somebody won
        ...
        with torch.no_grad():
            for i in range(num_processes):
                p = mp.Process(target=MCTS_self_play,
                               args=(net, args.num_games_per_MCTS_process, start_idx, i, args, iteration))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
